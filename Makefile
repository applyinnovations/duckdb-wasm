...
bench_internal: bench_build
	yarn workspace @applyinnovations/benchmarks bench:internal
...
js_debug: build/bootstrap yarn_install
	yarn workspace @applyinnovations/duckdb-wasm build:debug

# Build the duckdb library in release mode
.PHONY: js_release
js_release: yarn_install
	yarn workspace @applyinnovations/duckdb-wasm build:release

# Build the duckdb docs
.PHONY: docs
docs: yarn_install
	yarn workspace @applyinnovations/duckdb-wasm docs
...
ifeq (${DUCKDB_WASM_LOADABLE_EXTENSIONS}, 1)
	...
	yarn workspace @applyinnovations/duckdb-wasm test:chrome || echo "--- Parquet tests expected to fail!"
else
	yarn workspace @applyinnovations/duckdb-wasm test
endif
...
js_tests_browser: js_debug build/data
	yarn workspace @applyinnovations/duckdb-wasm test:chrome

# Run the duckdb javascript tests in browser
.PHONY: js_tests_browser_debug
js_tests_browser_debug: js_debug build/data
	yarn workspace @applyinnovations/duckdb-wasm test:browser:debug

# Run the duckdb javascript tests on nodejs
.PHONY: js_tests_node
js_tests_node: js_debug build/data
	yarn workspace @applyinnovations/duckdb-wasm test:node --filter=${JS_FILTER}

.PHONY: js_tests_node_debug
js_tests_node_debug: js_debug build/data
	yarn workspace @applyinnovations/duckdb-wasm test:node:debug --filter=${JS_FILTER}

wasmpack: yarn_install
	yarn workspace @applyinnovations/duckdb-wasm-shell install:wasmpack

.PHONY: shell
shell: build/bootstrap js_debug
	yarn workspace @applyinnovations/duckdb-wasm-shell build:debug

.PHONY: shell_release
shell_release: js_release
	yarn workspace @applyinnovations/duckdb-wasm-shell build:release

.PHONY: app_start
app_start: yarn_install
	yarn workspace @applyinnovations/duckdb-wasm-app start

.PHONY: app_start_corp
app_start_corp:
	yarn workspace @applyinnovations/duckdb-wasm-app start:corp

.PHONY: app
app: wasm wasmpack shell docs js_tests_release
	yarn workspace @applyinnovations/duckdb-wasm-app build:release
...
.PHONY: eslint
eslint:
	yarn workspace @applyinnovations/duckdb-wasm run lint
	yarn workspace @applyinnovations/duckdb-wasm-shell run lint
	yarn workspace @applyinnovations/benchmarks run lint
...
examples: yarn_install
	yarn workspace @applyinnovations/duckdb-wasm-examples-bare-node test
	yarn workspace @applyinnovations/duckdb-wasm-examples-bare-browser build
	yarn workspace @applyinnovations/duckdb-wasm-examples-esbuild-node build
	yarn workspace @applyinnovations/duckdb-wasm-examples-esbuild-node test
	yarn workspace @applyinnovations/duckdb-wasm-examples-esbuild-browser build
